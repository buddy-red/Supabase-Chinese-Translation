...
- [Supabase CLI](https://www.supabase.cc/docs/guides/cli) installed on your machine
...
There is quite a bit of stuff going on here, but basically, it’s creating a simple UI for the user to sign in using the [magic link](https://www.supabase.cc/docs/guides/auth/auth-magic-link), and once the user is signed in, will initialize OneSignal to ask the user to receive notifications on the website.
...
Let’s create an edge function that will receive [database webhooks](https://www.supabase.cc/docs/guides/database/webhooks) from the database and calls the OneSignal API to send the push notification.
