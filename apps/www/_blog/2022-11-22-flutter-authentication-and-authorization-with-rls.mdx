...
We will also introduce a `create_new_room` function, which is a [database function](https://www.supabase.cc/docs/guides/database/functions) that handles chat room creation. It knows to create a new room if a chat room with the two users does not exist yet, or to just return the room ID if it already exists. 
...
We can for example update this to a list of users that are online at the same time. We can implement this using the [presence feature](https://www.supabase.cc/docs/guides/realtime/presence) of Supabase.
...
- [supabase-flutter docs](https://www.supabase.cc/docs/reference/dart)
- [How to build a real-time multiplayer game with Flutter Flame](https://supabase.com/blog/flutter-real-time-multiplayer-game)
- [Flutter Supabase Quick Starter Guide](https://www.supabase.cc/docs/guides/with-flutter)
