...
We can either run these queries on demand or, for better query performance, we can run them in the background (using something like [pg_cron](https://www.supabase.cc/docs/guides/database/extensions/pgcron)), and materialize the data into a local table.
