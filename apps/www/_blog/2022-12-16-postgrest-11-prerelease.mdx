...
but the `not null` filter is more flexible and can be used with an [or filter](https://www.supabase.cc/docs/reference/javascript/or) to combine related tables' conditions.

## Try it out

This pre-release is not deployed to Supabase cloud but you can try it out locally with the [Supabase CLI](https://www.supabase.cc/docs/reference/cli/introduction).
